...
def get_annotation_component():
    ...
    component_dir = os.path.join(tempfile.gettempdir(), "pdf_annotation_component_v7")
    os.makedirs(component_dir, exist_ok=True)
    index_path = os.path.join(component_dir, "index.html")
...
const TOOLS=['select','text','rectangle','circle','line','arrow','pen'];
let args={},objects=[],selectedId=null,action=null,draft=null,undoStack=[],activeEditor=null,suppressNextTextClick=false;
...
function renderToolbar(){toolbar.innerHTML='';let g=group();const prev=control('button',{textContent:'◀',className:'small-btn',title:'Previous page'});prev.disabled=Number(args.page_num)<=0;prev.onclick=()=>emit({requested_page:Number(args.page_num)-1});g.appendChild(prev);const page=control('select');for(let i=0;i<Number(args.page_count||1);i++){const op=control('option',{textContent:`Page ${i+1} of ${args.page_count}`,value:String(i)});if(i===Number(args.page_num))op.selected=true;page.appendChild(op);}page.onchange=()=>emit({requested_page:Number(page.value)});g.appendChild(page);const next=control('button',{textContent:'▶',className:'small-btn',title:'Next page'});next.disabled=Number(args.page_num)>=Number(args.page_count)-1;next.onclick=()=>emit({requested_page:Number(args.page_num)+1});g.appendChild(next);
 g=group();TOOLS.forEach(t=>{const b=control('button',{textContent:t[0].toUpperCase()+t.slice(1),className:'tool'+(settings.tool===t?' active':'')});b.onclick=()=>{if(activeEditor)activeEditor.finish(true);settings.tool=t;renderToolbar();};g.appendChild(b);});
...
"""
    ...
    return components.declare_component("pdf_annotation_component_v7", path=component_dir)


def show_annotation_editor(page_num, page_count, page_image, annotations, selected_id, undo_stack, settings):
    ...
    return component(
        ...
        settings=settings,
        key=f"annotation_editor_v6_page_{page_num}",
        default=None,
    )
...
def render_annotate_tool():
    ...
    st.caption(
        ...
        "The page selector, tools, colours, thickness, font controls, and editing actions are all in the toolbar."
    )
...
def get_split_preview_component():
    """Create/load the isolated horizontal page-selection component."""
    component_dir = os.path.join(tempfile.gettempdir(), "split_pdf_preview_component_v1")
    os.makedirs(component_dir, exist_ok=True)
    index_path = os.path.join(component_dir, "index.html")
...
<script>
    ...
    function render(args) {
        ...
        currentPages.forEach(function(page) {
            ...
            checkbox.addEventListener("change", commitSelection);
...
"""
    ...
    return components.declare_component("split_pdf_preview_component", path=component_dir)
...
def render_split_tool():
    """Render the fully isolated page-extraction workflow."""
    st.header("Split PDF")
    ...
    st.success(f"Loaded PDF with {page_count} page(s).")
    st.write(
        "Scroll left and right through the preview. Check each page that should be included "
        "in the new excerpt PDF. Selected pages remain in their original order."
    )
    ...
    if selected_pages:
        display_pages = ", ".join(str(page_num + 1) for page_num in selected_pages)
        st.info(f"Selected {len(selected_pages)} page(s): {display_pages}")
    else:
        st.warning("No pages are currently selected.")

    if st.button("Generate Excerpt PDF", key="split_generate", disabled=not selected_pages):
