# PDF Toolkit Version 0.8 - Fabric annotation scale/refresh fix
# PDF Toolkit - v0.7
...
def get_annotation_component():
    ...
    component_dir = os.path.join(tempfile.gettempdir(), "pdf_fabric_annotation_component_v8")
...
function normalizeScaledObject(obj) {
  // Normalize only the object Fabric has just modified. Do NOT call this from
  // serializeObject(), because mutating every object during a save is what made
  // unrelated annotations drift when one selected object was resized/moved.
  const kind = obj.kind || "";
  const sx = Number(obj.scaleX || 1), sy = Number(obj.scaleY || 1);
  if (Math.abs(sx - 1) <= 0.001 && Math.abs(sy - 1) <= 0.001) return;

  // Preserve the visual center while baking Fabric scale into true dimensions.
  // Without this, setting scale back to 1 changes the object's anchor math and
  // produces small but cumulative position shifts on redraw.
  const center = obj.getCenterPoint();

  if (kind === "textbox") {
    obj.set({
      fontSize: Math.max(1, Math.round(Number(obj.fontSize || 18) * Math.max(sx, sy))),
      scaleX: 1,
      scaleY: 1
    });
  } else if (kind === "rectangle") {
    obj.set({
      width: Math.max(1, Number(obj.width || 1) * sx),
      height: Math.max(1, Number(obj.height || 1) * sy),
      scaleX: 1,
      scaleY: 1
    });
  } else if (kind === "circle") {
    obj.set({
      rx: Math.max(1, Number(obj.rx || 1) * sx),
      ry: Math.max(1, Number(obj.ry || 1) * sy),
      scaleX: 1,
      scaleY: 1
    });
  }
...
function serializeObject(obj) {
  // Pure serialization only. This function must not mutate canvas objects.
  const kind = obj.kind || "textbox";
  if (kind === "textbox") return { id: obj.id || makeId(), kind: "textbox", x0: Number(obj.left || 0), y0: Number(obj.top || 0), x1: Number(obj.left || 0) + Number(obj.width || 1) * Number(obj.scaleX || 1), y1: Number(obj.top || 0) + Number(obj.height || 1) * Number(obj.scaleY || 1), angle: Number(obj.angle || 0), text: obj.text || "", custom_color: obj.custom_color || obj.fill || "#ff0000", line_width: 1, font_family: obj.font_family || (obj.fontFamily === "Arial" ? "Arial" : "Sans Serif"), font_size: Number(obj.fontSize || 18), bold: obj.fontWeight === "bold", italic: obj.fontStyle === "italic", underline: obj.underline === true };
  ...
  if (kind === "rectangle") return { id: obj.id || makeId(), kind: kind, x0: Number(obj.left || 0), y0: Number(obj.top || 0), x1: Number(obj.left || 0) + Number(obj.width || 1) * Number(obj.scaleX || 1), y1: Number(obj.top || 0) + Number(obj.height || 1) * Number(obj.scaleY || 1), angle: Number(obj.angle || 0), custom_color: obj.custom_color || obj.stroke || "#ff0000", line_width: Number(obj.line_width || obj.strokeWidth || 3) };
  if (kind === "circle") return { id: obj.id || makeId(), kind: kind, x0: Number(obj.left || 0), y0: Number(obj.top || 0), x1: Number(obj.left || 0) + 2 * Number(obj.rx || 1) * Number(obj.scaleX || 1), y1: Number(obj.top || 0) + 2 * Number(obj.ry || 1) * Number(obj.scaleY || 1), angle: Number(obj.angle || 0), custom_color: obj.custom_color || obj.stroke || "#ff0000", line_width: Number(obj.line_width || obj.strokeWidth || 3) };
  return null;
}
...
"""
    ...
    return components.declare_component("pdf_fabric_annotation_component_v8", path=component_dir)
...
def show_draggable_annotation_page(page_num, base_image, annotations, settings, selected_id=None, caption=None):
    ...
    result = component(
        ...
        key=f"pdf_fabric_annotation_component_v8_page_{page_num}",
        default=None,
    )
        ...
        if event is not None:
            try:
                ...
                if (
                    ...
                    # Do not force an immediate rerun here. The Fabric canvas already has
                    # the current object state in the browser. Forcing a second Streamlit
                    # rerun reloads the iframe from server arguments and causes visible
                    # flashing and, when stale args win the race, apparent object rollback.
            except Exception as e:
