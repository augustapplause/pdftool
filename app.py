...
def extract_ocr_words_from_page(page, render_zoom=3.0, languages="eng+fra"):
    """OCR one PDF page and return PyMuPDF-compatible word tuples.

    OCR coordinates are converted from rendered-image pixels back into PDF
    page coordinates so the existing ruler boundaries and row grouping can be
    used without changing the rest of the XLSX extraction pipeline.
    """
    pix = page.get_pixmap(
    ...
    image = ImageOps.autocontrast(ImageOps.grayscale(source_image))
    ...
    for index, raw_text in enumerate(data.get("text", [])):
        ...
        # Discard very low-confidence noise while keeping uncertain but useful
        # characters in degraded scans.
        if confidence < 20:
            continue
...
def get_page_words_with_ocr_fallback(page):
    """Use native PDF words first, then OCR only when none are available."""
    native_words = page.get_text("words")
    if native_words:
        return native_words, False

    return extract_ocr_words_from_page(page), True
...
def render_xlsx_tool():
    ...
    with right_col:
        ...
        st.info(
            ...
            "Pages without selectable text will be OCR-processed automatically."
        )
    ...
    with left_col:
        ...
        if st.button("Generate XLSX", key="xlsx_generate"):
            ...
            try:
                ...
                with pd.ExcelWriter(output_xlsx, engine="openpyxl") as writer:
                    ...
                    for pnum in range(len(source_doc)):
                        ...
                        try:
                            page_words, used_ocr = get_page_words_with_ocr_fallback(page)
            ...
            if ocr_pages:
                ...
                st.info(f"OCR was automatically used on page(s): {page_list}.")
            else:
