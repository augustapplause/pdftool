...
def get_annotation_component():
    ...
    component_dir = os.path.join(tempfile.gettempdir(), "pdf_annotation_component_v1")
    os.makedirs(component_dir, exist_ok=True)
    index_path = os.path.join(component_dir, "index.html")
...
#scroll { width:100%; height:720px; overflow:auto; border:1px solid #bbb; box-sizing:border-box; background:#ddd; }
...
let args={}, objects=[], selectedId=null, action=null, draft=null;
const stage=document.getElementById('stage'), img=document.getElementById('page'), svg=document.getElementById('overlay');
...
function emit(){ msg('streamlit:setComponentValue',{value:{annotations:objects,selected_id:selectedId,nonce:Date.now()+'_'+Math.random().toString(36).slice(2)},dataType:'json'}); }
...
function color(){ return args.color||'#000000'; }
...
function objectNode(o){
 ...
 if(o.type==='rectangle') n=el('rect',Object.assign({x:o.x,y:o.y,width:o.w,height:o.h},common));
   ...
   const at=o.type==='arrow'?'url(#arrowhead)':'';
   n=el('line',Object.assign({x1:o.x1,y1:o.y1,x2:o.x2,y2:o.y2,'marker-end':at},common));
 }
 ...
 g.addEventListener('pointerdown', e=>{ if(args.tool!=='select') return; e.stopPropagation(); selectedId=o.id; const p=point(e); action={kind:'move',id:o.id,start:p,original:JSON.parse(JSON.stringify(o))}; svg.setPointerCapture(e.pointerId); render(); });
 return g;
}
function selectionNodes(o){
 const out=[], b=bbox(o), c=center(o), pad=5;
 const outline=el('rect',{x:b.x-pad,y:b.y-pad,width:b.w+2*pad,height:b.h+2*pad,class:'selected-outline',transform:transform(o)}); out.push(outline);
 if(o.type==='line'||o.type==='arrow'){
   [['p1',o.x1,o.y1],['p2',o.x2,o.y2]].forEach(([kind,x,y])=>{ const h=el('circle',{cx:x,cy:y,r:6,class:'endpoint'}); h.addEventListener('pointerdown',e=>{e.stopPropagation();action={kind,id:o.id,original:JSON.parse(JSON.stringify(o))};svg.setPointerCapture(e.pointerId)}); out.push(h); });
 } else {
   const corners=[['nw',b.x,b.y],['ne',b.x+b.w,b.y],['sw',b.x,b.y+b.h],['se',b.x+b.w,b.y+b.h]];
   corners.forEach(([kind,x,y])=>{ const h=el('rect',{x:x-5,y:y-5,width:10,height:10,class:'handle'}); h.addEventListener('pointerdown',e=>{e.stopPropagation();action={kind:'resize',corner:kind,id:o.id,original:JSON.parse(JSON.stringify(o)),start:point(e)};svg.setPointerCapture(e.pointerId)}); out.push(h); });
 }
 const ry=b.y-28; out.push(el('line',{x1:c.x,y1:b.y-pad,x2:c.x,y2:ry+6,class:'rotate-line'}));
 const rh=el('circle',{cx:c.x,cy:ry,r:7,class:'rotate-handle'}); rh.addEventListener('pointerdown',e=>{e.stopPropagation();action={kind:'rotate',id:o.id,center:c};svg.setPointerCapture(e.pointerId)}); out.push(rh);
 return out;
}
function render(){
 svg.innerHTML='';
 const defs=el('defs'); const marker=el('marker',{id:'arrowhead',markerWidth:10,markerHeight:8,refX:9,refY:4,orient:'auto',markerUnits:'strokeWidth'}); marker.appendChild(el('path',{d:'M0,0 L10,4 L0,8 Z',fill:'context-stroke'})); defs.appendChild(marker); svg.appendChild(defs);
...
function addText(p){
 const text=window.prompt('Enter annotation text:',''); if(text===null||text==='') return;
 const fs=Number(args.font_size||18), w=Math.max(40,text.length*fs*0.62), h=fs*1.35;
 objects.push({id:rid(),type:'text',x:p.x,y:p.y,w:w,h:h,rotation:0,text:text,font:args.font||'Arial',font_size:fs,bold:!!args.bold,italic:!!args.italic,underline:!!args.underline,color:color(),thickness:thickness()}); selectedId=objects[objects.length-1].id; render(); emit();
}
svg.addEventListener('pointerdown',e=>{
 const p=point(e);
 if(args.tool==='select'){ if(e.target===svg){selectedId=null;render();} return; }
 if(args.tool==='text'){ addText(p); return; }
 if(['rectangle','circle','line','arrow'].includes(args.tool)){
   if(args.tool==='line'||args.tool==='arrow') draft={id:rid(),type:args.tool,x1:p.x,y1:p.y,x2:p.x,y2:p.y,rotation:0,color:color(),thickness:thickness()};
   else draft={id:rid(),type:args.tool,x:p.x,y:p.y,w:1,h:1,rotation:0,color:color(),thickness:thickness()};
   action={kind:'draw',start:p}; svg.setPointerCapture(e.pointerId); render(); return;
 }
 if(args.tool==='pen'){ draft={id:rid(),type:'pen',points:[p],rotation:0,color:color(),thickness:thickness()}; action={kind:'pen'};svg.setPointerCapture(e.pointerId);render(); }
});
svg.addEventListener('pointermove',e=>{
 ...
 else { const o=byId(action.id); if(!o)return;
...
window.addEventListener('keydown',e=>{ if((e.key==='Delete'||e.key==='Backspace')&&selectedId){objects=objects.filter(o=>o.id!==selectedId);selectedId=null;render();emit();} });
window.addEventListener('message',event=>{ if(!event.data||event.data.type!=='streamlit:render')return; args=event.data.args||{}; objects=JSON.parse(JSON.stringify(args.annotations||[])); selectedId=args.selected_id||null; img.src='data:image/png;base64,'+args.preview_b64; img.width=args.width;img.height=args.height;stage.style.width=args.width+'px';stage.style.height=args.height+'px';svg.setAttribute('width',args.width);svg.setAttribute('height',args.height);svg.setAttribute('viewBox',`0 0 ${args.width} ${args.height}`);render(); });
ready();
</script>
</body>
</html>
"""
    ...
    return components.declare_component("pdf_annotation_component", path=component_dir)


def show_annotation_editor(page_num, page_image, annotations, selected_id, controls):
    ...
    return component(
        ...
        **controls,
        key=f"annotation_editor_page_{page_num}",
        default=None,
    )
...
def apply_annotations_to_page(page, annotations):
    ...
    for item in annotations:
        ...
        if kind in {"rectangle", "circle"}:
            ...
            points = rotated_points_for_annotation(item)
            pdf_points = [display_point_to_pdf(page, x, y) for x, y in points]
            rect = fitz.Rect(pdf_points[0], pdf_points[2]).normalize()
            style = (bool(item.get("bold")), bool(item.get("italic")))
            font_map = {(False, False): "Helv", (True, False): "HeBo", (False, True): "HeIt", (True, True): "HeBI"}
            rotation = int(round(float(item.get("rotation", 0)) / 90) * 90) % 360
            annot = page.add_freetext_annot(
                rect,
                str(item.get("text", "")),
                fontsize=max(4, float(item.get("font_size", 18)) / ZOOM),
                fontname=font_map[style],
                text_color=color,
                fill_color=None,
                border_color=None,
                rotate=rotation,
            ...
            if item.get("underline"):
                start = display_point_to_pdf(page, item["x"], item["y"] + item.get("h", 20))
                end = display_point_to_pdf(page, item["x"] + item.get("w", 40), item["y"] + item.get("h", 20))
                underline = page.add_line_annot(start, end)
                underline.set_colors(stroke=color)
                underline.set_border(width=max(0.5, thickness / 2))
                underline.update()
...
def render_annotate_tool():
    ...
    prepared = prepare_mode_pdf(MODE_ANNOTATE)
    if prepared is None:
        return
    ...
    tool_col, color_col, font_col, thickness_col = st.columns([2, 1, 1, 2])
    with tool_col:
    ...
    with color_col:
        color_name = st.selectbox("Colour", ["Black", "Red", "Blue", "Green", "Orange", "Purple", "White"], key="annotate_color")
    with font_col:
    ...
    color_map = {"Black": "#000000", "Red": "#d00000", "Blue": "#0057d9", "Green": "#008a2e", "Orange": "#e56b00", "Purple": "#7a32a8", "White": "#ffffff"}
    result = show_annotation_editor(
        page_num,
        page_image,
        pages[page_num],
        selected_by_page.get(page_num),
        {"tool": tool, "color": color_map[color_name], "font": font, "thickness": thickness, "font_size": font_size, "bold": bold, "italic": italic, "underline": underline},
    )
    st.caption("Select an object to move, resize, or rotate it. Press Delete or use Delete Selected to remove only that object.")
