...
def ocr_page_words_by_columns(
    ...
    """OCR each marked table column separately using exact rendered geometry.

    Each crop includes a small horizontal padding area to avoid clipping digits
    or letters that touch a ruler boundary. Detections are still accepted only
    when their midpoint lies inside the unpadded core column, so padding cannot
    leak values into neighbouring output columns.
    """
    reader = get_ocr_reader()
    if reader is None:
        raise RuntimeError(
    ...
    render_zoom = max(float(render_zoom or 2.25), 1.5)
    pix = page.get_pixmap(matrix=fitz.Matrix(render_zoom, render_zoom), alpha=False)
    page_image = Image.open(BytesIO(pix.tobytes("png"))).convert("RGB")
    page_width_px, page_height_px = page_image.size

    displayed_width_pt = max(float(page.rect.width), 0.0001)
    displayed_height_pt = max(float(page.rect.height), 0.0001)
    x_pixels_per_point = page_width_px / displayed_width_pt
    y_pixels_per_point = page_height_px / displayed_height_pt

    all_words = []
    diagnostics = {
        "x_pixels_per_point": x_pixels_per_point,
        "y_pixels_per_point": y_pixels_per_point,
    ...
    if include_crop_diagnostics:
        diagnostics["map_png"] = _make_crop_diagnostic_map(
            page_image, boundaries, x_pixels_per_point
        )

    try:
        for col_index in range(len(boundaries) - 1):
            core_left_pt = max(0.0, float(boundaries[col_index]))
            core_right_pt = min(displayed_width_pt, float(boundaries[col_index + 1]))
            if core_right_pt - core_left_pt < 1.0:
                continue

            # Four PDF points is enough to protect glyph edges without allowing
            # neighbouring columns to dominate the OCR crop.
            padding_pt = min(6.0, max(2.0, (core_right_pt - core_left_pt) * 0.04))
            crop_left_pt = max(0.0, core_left_pt - padding_pt)
            crop_right_pt = min(displayed_width_pt, core_right_pt + padding_pt)

            core_left_px = int(round(core_left_pt * x_pixels_per_point))
            core_right_px = int(round(core_right_pt * x_pixels_per_point))
            crop_left_px = max(0, int(round(crop_left_pt * x_pixels_per_point)))
            crop_right_px = min(page_width_px, int(round(crop_right_pt * x_pixels_per_point)))

            if crop_right_px - crop_left_px < 4:
                continue
            ...
            try:
                ...
                target_width = 700
                upscale_factor = min(2.25, max(1.0, target_width / crop_width))
                ...
                if include_crop_diagnostics:
                    ...
                    try:
                        diagnostics["crops"].append({
                            "column": col_index + 1,
                            "core_left_pt": core_left_pt,
                            "core_right_pt": core_right_pt,
                            "crop_left_pt": crop_left_pt,
                            "crop_right_pt": crop_right_pt,
                ...
                general_words = easyocr_column_results_to_words(
                    general_results,
                    x_pixels_per_point,
                    y_pixels_per_point,
                    crop_left_px,
                ...
                numeric_words = easyocr_column_results_to_words(
                    numeric_results,
                    x_pixels_per_point,
                    y_pixels_per_point,
                    crop_left_px,
                ...
                for word in column_words:
                    ...
                    if core_left_pt <= midpoint <= core_right_pt:
...
def get_words_with_column_ocr_fallback(
    ...
    ocr_words, crop_diagnostics = ocr_page_words_by_columns(
        ...
        render_zoom=max(float(zoom), 2.25),
    ...
    if status_area is not None:
        status_area.success(
            ...
            f"{len(ocr_words)} text items across {len(boundaries) - 1} columns "
