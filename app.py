...
# Changelog
# v3.5
# - Fixed XLSX diagnostic/table reconstruction NameError by consistently using active_boundaries.
# v3.4
# - Column OCR now crops directly from the same rendered pixel image shown in the XLSX preview.
# - User ruler positions are used as pixel boundaries with no PDF-point or rotation conversion.
# - Diagnostic crop maps and samples now exactly match the image regions sent to EasyOCR.
# v3.3
# - Fixed column OCR crop geometry by using the rendered pixmap's exact X/Y scale.
# - Added small horizontal crop padding so column boundaries do not clip glyphs.
# - Diagnostic workbooks now include CropMap and CropSamples sheets showing the exact OCR regions.
# v3.2
# - XLSX OCR now processes each user-defined column separately instead of OCRing the whole page.
# - Narrow column crops are upscaled before OCR to improve single- and double-digit recognition.
# - Numeric recovery pass added for column crops while preserving diagnostic sheets.
# v3.1
# - Diagnostic XLSX release: exports raw OCR detections, grouped rows, and final column assignments.
# - EasyOCR confidence values are retained in diagnostic sheets.
# v3.0
# - Original uploaded PDF is preserved unchanged in session memory.
# - A separate working PDF is used by Redact, Annotate, XLSX, and Plain Text.
# - Rotate Left / Rotate Right modify the current page in the working PDF.
# - Removed the XLSX-only raster rotation path and unstable highest-accuracy OCR mode.
# - Rotated working pages use the normal rendered-page OCR path.
# v2.2
...
def clear_page_markup_state(page_num):
    """Clear coordinate-based markup that would no longer align after page rotation."""
...
def rotate_working_pdf_page(page_num, delta_degrees):
    """Rotate one page in the in-memory working PDF while preserving the original upload."""
    source_bytes = st.session_state.get("working_pdf_bytes")
    if not source_bytes:
        raise RuntimeError("No working PDF is loaded.")
    ...
    try:
        ...
        if page_num < 0 or page_num >= len(doc):
            raise IndexError("Page number is outside the working PDF.")
        page = doc[page_num]
        page.set_rotation((int(page.rotation) + int(delta_degrees)) % 360)
        doc.save(output, garbage=3, deflate=True)
        st.session_state["working_pdf_bytes"] = output.getvalue()
        clear_page_markup_state(page_num)
        st.session_state["rotation_notice"] = (
            f"Page {page_num + 1} rotated in the working PDF. "
            "Existing coordinate-based markup on that page was cleared to prevent misalignment."
        )
    finally:
        if doc is not None:
            doc.close()
...
def restore_original_working_pdf():
    """Restore the untouched upload as the working PDF and clear coordinate-based edits."""
    original = st.session_state.get("original_pdf_bytes")
    if not original:
        return
    st.session_state["working_pdf_bytes"] = original
    st.session_state.redactions = {}
    st.session_state.columns = {}
    st.session_state.annotations = {}
    st.session_state.annotation_overlays = {}
    ...
    st.session_state.last_redaction_click = {}
    st.session_state.last_annotation_click = {}
    st.session_state["rotation_notice"] = "Working PDF restored from the untouched original upload."
...
if uploaded is not None:
    ...
    if uploaded_hash != st.session_state.get("uploaded_pdf_hash"):
        st.session_state["uploaded_pdf_hash"] = uploaded_hash
        st.session_state["original_pdf_bytes"] = uploaded_bytes
        st.session_state["working_pdf_bytes"] = uploaded_bytes
...
if uploaded is None and "working_pdf_bytes" not in st.session_state:
    st.stop()
...
pdf_bytes = st.session_state["working_pdf_bytes"]
...
def assign_word_to_column(word, active_boundaries):
    x0, y0, x1, y1, text, *_ = word
    x_mid = (x0 + x1) / 2

    for i in range(len(active_boundaries) - 1):
        if active_boundaries[i] <= x_mid < active_boundaries[i + 1]:
            return i

    return None


def build_raw_detection_diagnostics(words, active_boundaries):
    """Return one row per extracted word before row grouping."""
    records = []
    for detection_index, word in enumerate(words or []):
        x0, y0, x1, y1, text, *extra = word
        confidence = None
        if len(word) >= 9:
            try:
                confidence = float(word[8])
            except Exception:
                confidence = None
        column_index = assign_word_to_column(word, active_boundaries)
        records.append({
            ...
            "x_mid": (x0 + x1) / 2,
            "y_mid": (y0 + y1) / 2,
            "assigned_column": None if column_index is None else column_index + 1,
        })
    return pd.DataFrame(records)


def build_grouped_row_diagnostics(rows, active_boundaries):
    """Return one row per grouped line, showing every word and assembled columns."""
    records = []
    column_count = max(0, len(boundaries) - 1)
    for row_index, row in enumerate(rows or []):
        sorted_words = sorted(row.get("words", []), key=lambda w: w[0])
        cells = [""] * column_count
        word_details = []
        for word in sorted_words:
            column_index = assign_word_to_column(word, active_boundaries)
            if column_index is not None:
                cells[column_index] = (cells[column_index] + " " + str(word[4])).strip()
            word_details.append(
                f"{word[4]} [x={((word[0] + word[2]) / 2):.2f}, y={((word[1] + word[3]) / 2):.2f}, col={'' if column_index is None else column_index + 1}]"
            )
        record = {
            "grouped_row_index": row_index,
            "group_y": row.get("y"),
            "word_count": len(sorted_words),
            "all_words": " | ".join(word_details),
        }
        for column_index, value in enumerate(cells, start=1):
            record[f"column_{column_index}"] = value
...
def easyocr_results_to_words(ocr_results, zoom):
    ...
    for idx, result in enumerate(ocr_results):
        ...
        words.append((x0, y0, x1, y1, text_value, 0, idx, idx, float(confidence)))
...
def get_words_for_xlsx_extraction(page, pnum, zoom, rotation_degrees, status_area=None, ocr_quality_mode="High Accuracy", ocr_zoom=None):
    ...
    # Page rotation is now applied to the working PDF itself. PyMuPDF renders the
    # page upright, but selectable-text coordinates can remain tied to the
    # unrotated page coordinate system. Use the normal rendered-page OCR path for
    # rotated pages so ruler coordinates and extracted words share one orientation.
    if int(getattr(page, "rotation", 0) or 0) % 360:
        if status_area is not None:
            status_area.info(
                f"Page {pnum + 1}: working page rotation is {page.rotation}°. "
                "Using rendered-page OCR so extraction matches the visible orientation."
            )
        ocr_start = time.perf_counter()
        ocr_words = ocr_page_words(page, max(float(zoom), 1.75))
        ocr_seconds = time.perf_counter() - ocr_start
        total_seconds = time.perf_counter() - page_start
        if status_area is not None:
            status_area.success(
                f"Page {pnum + 1}: rotated-page OCR used. "
                f"{len(ocr_words or [])} text items in {ocr_seconds:.2f}s."
            )
        return ocr_words, "ocr", {
            "selectable_words": selectable_count,
            "ocr_words": len(ocr_words or []),
...
def easyocr_column_results_to_words(
    ocr_results,
    x_pixels_per_point,
    y_pixels_per_point,
    crop_left_px,
    ...
    """Map OCR results from an upscaled crop back to displayed PDF coordinates.

    v3.3 deliberately uses the pixmap's measured X and Y scales instead of
    assuming they exactly equal the requested render zoom. This matters for
    rotated pages, where rounding and page geometry can otherwise make crop
    coordinates drift away from the ruler positions.
    """
    words = []
    safe_upscale = max(float(upscale_factor or 1.0), 0.0001)
    safe_x_scale = max(float(x_pixels_per_point or 1.0), 0.0001)
    safe_y_scale = max(float(y_pixels_per_point or 1.0), 0.0001)

    for idx, result in enumerate(ocr_results or []):
        try:
            box, text_value, confidence = result
        except Exception:
            continue
        ...
        x0_px = float(crop_left_px) + (min(xs) / safe_upscale)
        x1_px = float(crop_left_px) + (max(xs) / safe_upscale)
        y0_px = float(crop_top_px) + (min(ys) / safe_upscale)
        y1_px = float(crop_top_px) + (max(ys) / safe_upscale)
...
def merge_word_detections(primary_words, secondary_words, x_tolerance=3.0, y_tolerance=3.0):
    """Merge OCR word tuples while suppressing same-text detections at the same location."""
    merged = list(primary_words or [])

    for candidate in secondary_words or []:
        try:
            cx = (float(candidate[0]) + float(candidate[2])) / 2
            cy = (float(candidate[1]) + float(candidate[3])) / 2
            text = str(candidate[4]).strip().lower()
        except Exception:
            continue
        ...
        for existing in merged:
            try:
                ex = (float(existing[0]) + float(existing[2])) / 2
                ey = (float(existing[1]) + float(existing[3])) / 2
                etext = str(existing[4]).strip().lower()
            except Exception:
                continue
            if text == etext and abs(cx - ex) <= x_tolerance and abs(cy - ey) <= y_tolerance:
                duplicate = True
                break
...
def _make_crop_diagnostic_map(page_image, boundaries, x_pixels_per_point):
    """Return a scaled PNG showing the exact core column boundaries used by OCR."""
    diagnostic = page_image.copy()
    draw = ImageDraw.Draw(diagnostic)
    for index, boundary in enumerate(boundaries):
        x = int(round(float(boundary) * x_pixels_per_point))
        x = max(0, min(diagnostic.width - 1, x))
        draw.line([(x, 0), (x, diagnostic.height)], fill=(255, 0, 0), width=3)
        draw.rectangle([(x + 3, 3), (x + 42, 23)], fill=(255, 255, 255))
        draw.text((x + 5, 5), str(index), fill=(255, 0, 0))

    max_width = 1200
    if diagnostic.width > max_width:
        ratio = max_width / diagnostic.width
        resized = diagnostic.resize(
            (max_width, max(1, int(diagnostic.height * ratio))),
    ...
    try:
        return _image_to_png_bytes(diagnostic)
    finally:
        diagnostic.close()


def easyocr_pixel_results_to_words(
    ocr_results,
    crop_left_px,
    crop_top_px=0,
    upscale_factor=1.0,
):
    """Map OCR results from an upscaled pixel crop back to preview pixel coordinates."""
    words = []
    safe_upscale = max(float(upscale_factor or 1.0), 0.0001)
    ...
    for idx, result in enumerate(ocr_results or []):
        ...
        x0 = float(crop_left_px) + min(xs) / safe_upscale
        x1 = float(crop_left_px) + max(xs) / safe_upscale
        y0 = float(crop_top_px) + min(ys) / safe_upscale
        y1 = float(crop_top_px) + max(ys) / safe_upscale
...
def _make_pixel_crop_diagnostic_map(page_image, boundaries_px):
    """Show the exact preview-pixel boundaries used for OCR crops."""
    diagnostic = page_image.copy()
    draw = ImageDraw.Draw(diagnostic)
    for index, boundary in enumerate(boundaries_px):
        x = max(0, min(diagnostic.width - 1, int(round(float(boundary)))))
        draw.line([(x, 0), (x, diagnostic.height)], fill=(255, 0, 0), width=3)
        draw.rectangle([(x + 3, 3), (x + 48, 23)], fill=(255, 255, 255))
        draw.text((x + 5, 5), str(index), fill=(255, 0, 0))
...
def ocr_page_words_by_columns(
    page,
    boundaries_px,
    quality_mode="High Accuracy",
    render_zoom=1.5,
    include_crop_diagnostics=False,
):
    """OCR exact pixel columns from the same rendered image used by the ruler UI.

    The ruler stores X positions in preview pixels. This function renders the page
    at the same preview zoom and crops those exact pixels, avoiding all PDF-point,
    page-rotation, and scaling conversions.
    """
    reader = get_ocr_reader()
    if reader is None:
        raise RuntimeError(
    ...
    render_zoom = max(float(render_zoom or 1.5), 0.5)
    pix = page.get_pixmap(matrix=fitz.Matrix(render_zoom, render_zoom), alpha=False)
    page_image = Image.open(BytesIO(pix.tobytes("png"))).convert("RGB")
    page_width_px, page_height_px = page_image.size

    cleaned = [0.0]
    for value in boundaries_px[1:-1]:
        x = max(0.0, min(float(page_width_px), float(value)))
        if x > cleaned[-1] + 1:
            cleaned.append(x)
    if float(page_width_px) > cleaned[-1] + 1:
        cleaned.append(float(page_width_px))
    elif cleaned[-1] != float(page_width_px):
        cleaned[-1] = float(page_width_px)
    boundaries_px = cleaned
    ...
    if include_crop_diagnostics:
        diagnostics["map_png"] = _make_pixel_crop_diagnostic_map(page_image, boundaries_px)

    try:
        for col_index in range(len(boundaries_px) - 1):
            core_left_px = max(0, int(round(boundaries_px[col_index])))
            core_right_px = min(page_width_px, int(round(boundaries_px[col_index + 1])))
            if core_right_px - core_left_px < 4:
                continue

            padding_px = min(12, max(3, int((core_right_px - core_left_px) * 0.04)))
            crop_left_px = max(0, core_left_px - padding_px)
            crop_right_px = min(page_width_px, core_right_px + padding_px)

            crop = page_image.crop((crop_left_px, 0, crop_right_px, page_height_px))
            upscaled = None
            processed = None
            try:
                crop_width = max(1, crop.width)
                target_width = 700
                upscale_factor = min(3.0, max(1.0, target_width / crop_width))
                if upscale_factor > 1.01:
                    upscaled = crop.resize(
                        (
                            max(1, int(crop.width * upscale_factor)),
                            max(1, int(crop.height * upscale_factor)),
                        ),
                        Image.Resampling.LANCZOS,
                    )
                else:
                ...
                processed = prepare_image_for_ocr(upscaled, quality_mode)

                if include_crop_diagnostics:
                    sample = processed.copy()
                    max_sample_height = 700
                    if sample.height > max_sample_height:
                        ratio = max_sample_height / sample.height
                        resized = sample.resize(
                            (max(1, int(sample.width * ratio)), max_sample_height),
                            Image.Resampling.LANCZOS,
                        )
                        sample.close()
                        sample = resized
                    try:
                        diagnostics["crops"].append({
                            "column": col_index + 1,
                            "core_left_px": core_left_px,
                            "core_right_px": core_right_px,
                            "crop_left_px": crop_left_px,
                            "crop_right_px": crop_right_px,
                            "upscale_factor": upscale_factor,
                            "png": _image_to_png_bytes(sample),
                        })
                    finally:
                        sample.close()

                general_results = readtext_with_quality(reader, processed, quality_mode)
                general_words = easyocr_pixel_results_to_words(
                    general_results,
                    crop_left_px,
                    crop_top_px=0,
                    upscale_factor=upscale_factor,
                ...
                numeric_results = []
                try:
                    numeric_results = reader.readtext(
                        np.array(processed),
                        detail=1,
                        paragraph=False,
                        decoder="greedy",
                ...
                numeric_words = easyocr_pixel_results_to_words(
                    numeric_results,
                    crop_left_px,
                    crop_top_px=0,
                    upscale_factor=upscale_factor,
                )
                column_words = merge_word_detections(general_words, numeric_words)

                for word in column_words:
                    midpoint = (float(word[0]) + float(word[2])) / 2
                    if core_left_px <= midpoint <= core_right_px:
                        all_words.append(word)
            finally:
                crop.close()
                if upscaled is not None and upscaled is not crop:
                    upscaled.close()
                if processed is not None and processed is not upscaled:
                    processed.close()
                gc.collect()
    finally:
        page_image.close()
    ...
    return sorted(all_words, key=lambda w: (w[1], w[0])), diagnostics


def get_words_with_column_ocr_fallback(
    page,
    pnum,
    boundaries_px,
    zoom,
    ...
    """Use selectable text when reliable; otherwise OCR each marked column separately."""
    page_start = time.perf_counter()
    text_start = time.perf_counter()
    selectable_words = page.get_text("words")
    selectable_seconds = time.perf_counter() - text_start
    ...
    page_is_rotated = int(getattr(page, "rotation", 0) or 0) % 360 != 0
    if has_enough_selectable_words(selectable_words) and not page_is_rotated:
        total_seconds = time.perf_counter() - page_start
        if status_area is not None:
            status_area.success(
                f"Page {pnum + 1}: selectable text found. "
                f"{selectable_count} words, {selectable_seconds:.2f}s. "
                f"Column OCR skipped. Total {total_seconds:.2f}s."
            )
    ...
    if status_area is not None:
        reason = "working page is rotated" if page_is_rotated else f"only {selectable_count} selectable words were found"
        status_area.warning(
            f"Page {pnum + 1}: {reason}. Running column-by-column OCR..."
        )

    ocr_start = time.perf_counter()
    ocr_words, crop_diagnostics = ocr_page_words_by_columns(
        page,
        boundaries_px,
        quality_mode=quality_mode,
        render_zoom=float(zoom),
        include_crop_diagnostics=include_crop_diagnostics,
    )
    ocr_seconds = time.perf_counter() - ocr_start
    total_seconds = time.perf_counter() - page_start
    ...
    if status_area is not None:
        status_area.success(
            f"Page {pnum + 1}: column OCR used. "
            f"{len(ocr_words)} text items across {len(boundaries_px) - 1} columns "
            f"in {ocr_seconds:.2f}s. Total {total_seconds:.2f}s."
        )

    return ocr_words, "column_ocr", {
        "selectable_words": selectable_count,
...
def write_crop_diagnostic_sheets(writer, pnum, crop_diagnostics):
    """Embed the exact OCR map and crop samples in diagnostic workbook sheets."""
    ...
    buffers = getattr(writer, "_pdf_tool_image_buffers", None)
    if buffers is None:
        buffers = []
        setattr(writer, "_pdf_tool_image_buffers", buffers)

    map_png = crop_diagnostics.get("map_png")
    if map_png:
        map_name = f"CropMap_P{pnum + 1}"[:31]
        ws = writer.book.create_sheet(map_name)
        ws["A1"] = "Red lines and labels show the exact unpadded core column boundaries."
        ws["A2"] = f"X pixels/point: {crop_diagnostics.get('x_pixels_per_point', 0):.6f}"
        ws["A3"] = f"Y pixels/point: {crop_diagnostics.get('y_pixels_per_point', 0):.6f}"
        buffer = BytesIO(map_png)
        buffers.append(buffer)
        ws.add_image(XLImage(buffer), "A5")

    crops = crop_diagnostics.get("crops") or []
    if crops:
        sheet_name = f"CropSamples_P{pnum + 1}"[:31]
        ws = writer.book.create_sheet(sheet_name)
        row = 1
        for item in crops:
            ws.cell(row=row, column=1, value=f"Column {item['column']}")
            ws.cell(row=row + 1, column=1, value=(
                f"Core PDF x: {item['core_left_pt']:.2f} to {item['core_right_pt']:.2f}; "
                f"Padded crop x: {item['crop_left_pt']:.2f} to {item['crop_right_pt']:.2f}; "
                f"Upscale: {item['upscale_factor']:.2f}x"
            ))
            buffer = BytesIO(item["png"])
            buffers.append(buffer)
            image = XLImage(buffer)
            ws.add_image(image, f"A{row + 3}")
            image_rows = max(8, int((image.height or 300) / 20) + 2)
            row += image_rows + 5
...
with right_col:
    ...
    st.markdown("**Working PDF orientation**")
    rotate_left_col, rotate_right_col = st.columns(2)
    with rotate_left_col:
        if st.button("⟲ Rotate Left", key="working_pdf_rotate_left", use_container_width=True):
            rotate_working_pdf_page(page_num, -90)
            st.rerun()
    with rotate_right_col:
        if st.button("⟳ Rotate Right", key="working_pdf_rotate_right", use_container_width=True):
            rotate_working_pdf_page(page_num, 90)
            st.rerun()

    if st.button("Restore Untouched Original", key="restore_original_working_pdf", use_container_width=True):
        restore_original_working_pdf()
        st.rerun()

    if st.session_state.get("rotation_notice"):
        st.info(st.session_state.pop("rotation_notice"))
...
with left_col:
    if task == "Convert PDF to XLSX":
        ...
        st.caption(
            "Use Rotate Left / Rotate Right in Page Tools. Rotation changes the in-memory working PDF, "
            "so the preview, ruler, OCR, and all other PDF functions use the same orientation."
        )

        include_xlsx_diagnostics = st.checkbox(
            "Include OCR diagnostic sheets",
            value=True,
            key="xlsx_include_diagnostics",
            help=(
                "Adds Raw_P#, Rows_P#, and Summary sheets so we can see whether a missing value "
                "was lost during OCR, row grouping, or column assignment."
            ),
        )
        if include_xlsx_diagnostics:
            st.info(
                "Diagnostic mode is enabled. The workbook will include raw detections, grouped rows, "
                "and the normal final page sheets."
            )

        current_columns = st.session_state.columns.get(layout_page_num, [])

        xlsx_preview_image = page_image
        ...
        if st.button("Generate XLSX"):
            ...
            try:
                ...
                with pd.ExcelWriter(output_xlsx, engine="openpyxl") as writer:
                    ...
                    for pnum in range(total_pages):
                        ...
                        pdf_boundaries = [0] + pdf_columns + [p.rect.width]

                        # Ruler clicks are stored in the exact preview pixel coordinate
                        # system. Column OCR now consumes these directly.
                        preview_pix = p.get_pixmap(matrix=fitz.Matrix(zoom, zoom), alpha=False)
                        preview_width_px = preview_pix.width
                        pixel_columns = sorted(
                            max(0, min(preview_width_px, int(round(x))))
                            for x in raw_columns
                        )
                        pixel_boundaries = [0] + pixel_columns + [preview_width_px]

                        try:
                            with status_area:
                                page_status = st.empty()

                            words, extraction_method, extraction_stats = get_words_with_column_ocr_fallback(
                                p,
                                pnum,
                                pixel_boundaries,
                                zoom,
                                ...
                                quality_mode="High Accuracy",
                                include_crop_diagnostics=include_xlsx_diagnostics,
                            )
                        except Exception as e:
                            with status_area:
                        ...
                        if extraction_method in ("ocr", "column_ocr"):
                            pages_using_ocr += 1

                        active_boundaries = (
                            pixel_boundaries if extraction_method == "column_ocr" else pdf_boundaries
                        )
                        rows = group_words_into_rows(words)

                        if include_xlsx_diagnostics:
                            raw_diag_df = build_raw_detection_diagnostics(words, active_boundaries)
                            grouped_diag_df = build_grouped_row_diagnostics(rows, active_boundaries)
                            raw_diag_df.to_excel(
                                writer,
                                sheet_name=f"Raw_P{pnum + 1}"[:31],
                                index=False,
                            )
                            grouped_diag_df.to_excel(
                                writer,
                                sheet_name=f"Rows_P{pnum + 1}"[:31],
                                index=False,
                            )
                            write_crop_diagnostic_sheets(
                                writer,
                                pnum,
                                extraction_stats.get("crop_diagnostics"),
                            )
                            diagnostic_summary.append({
                                "page": pnum + 1,
                                "extraction_method": extraction_method,
                                "raw_detection_count": len(words or []),
                                "grouped_row_count": len(rows or []),
                                "column_marker_count": len(raw_columns),
                                "selectable_word_count": extraction_stats.get("selectable_words", 0),
                                "ocr_item_count": extraction_stats.get("ocr_words", 0),
                            })

                        table_rows = []

                        for row in rows:
                            cells = [""] * (len(active_boundaries) - 1)

                            for word in sorted(row["words"], key=lambda w: w[0]):
                                col_index = assign_word_to_column(word, active_boundaries)
                    ...
                    if include_xlsx_diagnostics and diagnostic_summary:
                        pd.DataFrame(diagnostic_summary).to_excel(
                            writer,
                            sheet_name="Diagnostic_Summary",
                            index=False,
                        )
                        wrote_sheet = True
            ...
            st.download_button(
                label="Download XLSX",
                data=output_bytes,
                file_name="extracted_diagnostic.xlsx" if include_xlsx_diagnostics else "extracted.xlsx",
