...
def draw_pdf_arrow(page, p0, p1, color, width):
    ...
    line_len = math.hypot(p1.x - p0.x, p1.y - p0.y) or 1
    shaft_trim = min(head_len * 0.78, line_len * 0.45)
    shaft_end = fitz.Point(
        p1.x - shaft_trim * math.cos(angle),
        p1.y - shaft_trim * math.sin(angle),
    )
    page.draw_line(p0, shaft_end, color=color, width=width)
    ...
    page.draw_polyline([a, p1, b], color=color, width=width)
