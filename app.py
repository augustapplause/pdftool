...
def ocr_page_words_for_xlsx_rotation(page, zoom, rotation_degrees):
    """Render the page in the same rotated orientation as the XLSX preview and OCR it.

    The normal OCR path renders the source PDF orientation. For sideways scans,
    rotating only the preview makes ruler clicks look correct but OCR still reads
    the sideways source image. This function OCRs the rotated preview image, so
    OCR word coordinates already match the rotated ruler/preview coordinate space.
    """
    reader = get_ocr_reader()
    ...
    try:
        ...
        ocr_results = reader.readtext(np.array(rotated_image), detail=1, paragraph=False)
    finally:
        image.close()
...
def get_words_for_xlsx_extraction(page, pnum, zoom, rotation_degrees, status_area=None):
    ...
    if status_area is not None:
        status_area.warning(
            ...
            f"({selectable_seconds:.2f}s). Running OCR on the rotated XLSX preview..."
        )

    ocr_start = time.perf_counter()
    ocr_words = ocr_page_words_for_xlsx_rotation(page, zoom, rotation_degrees)
