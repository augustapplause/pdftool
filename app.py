...
def easyocr_column_results_to_words(ocr_results, render_zoom, crop_left_px, upscale_factor=1.0):
    """Map OCR results from an upscaled column crop back to page/PDF coordinates."""
    words = []
    safe_upscale = max(float(upscale_factor or 1.0), 0.0001)
    safe_zoom = max(float(render_zoom or 1.0), 0.0001)
    ...
    for idx, result in enumerate(ocr_results or []):
        ...
        y0_px = min(ys) / safe_upscale
        y1_px = max(ys) / safe_upscale
...
def ocr_page_words_by_columns(page, boundaries, quality_mode="High Accuracy", render_zoom=2.25):
    """OCR each marked table column separately and return page-coordinate word tuples.

    Column crops reduce visual clutter. Narrow crops are enlarged before OCR so
    isolated one- and two-digit values occupy enough pixels for EasyOCR's text
    detector. A second numeric-only pass recovers small values that the general
    detector may classify as noise.
    """
    reader = get_ocr_reader()
    if reader is None:
        raise RuntimeError(
    ...
    try:
        for col_index in range(len(boundaries) - 1):
            left_pt = max(0.0, float(boundaries[col_index]))
            right_pt = min(float(page.rect.width), float(boundaries[col_index + 1]))
            if right_pt - left_pt < 1.0:
                continue

            left_px = max(0, int(round(left_pt * render_zoom)))
            right_px = min(page_width_px, int(round(right_pt * render_zoom)))
            if right_px - left_px < 4:
                continue

            crop = page_image.crop((left_px, 0, right_px, page_height_px))
            upscaled = None
            processed = None
            try:
                # Narrow numeric columns benefit most from enlargement. Keep the
                # cap modest so Streamlit Cloud memory remains predictable.
                ...
                if upscale_factor > 1.01:
                    upscaled = crop.resize(
                        (max(1, int(crop.width * upscale_factor)), max(1, int(crop.height * upscale_factor))),
                ...
                general_words = easyocr_column_results_to_words(
                    general_results, render_zoom, left_px, upscale_factor
                )

                # Numeric-only recovery pass. This is particularly effective for
                # isolated values such as 2, 4, 17, 39 and 43.
                numeric_results = []
                try:
                ...
                numeric_words = easyocr_column_results_to_words(
                    numeric_results, render_zoom, left_px, upscale_factor
                )
                column_words = merge_word_detections(general_words, numeric_words)

                # Keep detections assigned to this crop's true column span. This
                # prevents edge noise from leaking into a neighbouring column.
                for word in column_words:
                    midpoint = (float(word[0]) + float(word[2])) / 2
                    if left_pt <= midpoint <= right_pt:
    ...
    return sorted(all_words, key=lambda w: (w[1], w[0]))


def get_words_with_column_ocr_fallback(page, pnum, boundaries, zoom, status_area=None, quality_mode="High Accuracy"):
    ...
    # Rotated working pages and image-only pages use column OCR. Selectable text
    # remains the fastest and most accurate route for ordinary digital PDFs.
    ...
    ocr_start = time.perf_counter()
    ocr_words = ocr_page_words_by_columns(
        page,
        boundaries,
