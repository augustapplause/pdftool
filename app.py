...
def build_raw_detection_diagnostics(words, boundaries):
    ...
    for detection_index, word in enumerate(words or []):
        ...
        if len(word) >= 9:
            try:
                ...
                confidence = None
        column_index = assign_word_to_column(word, boundaries)
        records.append({
...
def build_grouped_row_diagnostics(rows, boundaries):
            ...
            try:
                ...
                with pd.ExcelWriter(output_xlsx, engine="openpyxl") as writer:
                    ...
                    for pnum in range(total_pages):
                        ...
                        if include_xlsx_diagnostics:
                            raw_diag_df = build_raw_detection_diagnostics(words, boundaries)
                            grouped_diag_df = build_grouped_row_diagnostics(rows, boundaries)
