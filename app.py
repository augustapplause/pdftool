...
def get_annotation_component():
    ...
    component_dir = os.path.join(tempfile.gettempdir(), "pdf_annotation_component_v2")
    os.makedirs(component_dir, exist_ok=True)
    index_path = os.path.join(component_dir, "index.html")
...
#more-color { width:34px; height:29px; border:0; padding:0; background:transparent; cursor:pointer; }
...
.inline-text { position:absolute; z-index:5; min-width:40px; border:0; border-bottom:1px dashed #1677ff; outline:none; padding:0; margin:0; background:transparent; line-height:1.15; }
...
let args={}, objects=[], selectedId=null, action=null, draft=null, activeColor='#000000', undoStack=[];
...
function arrowPolygon(o){
 ...
 const headLength=Math.max(8,Number(o.thickness||2)*4.2), halfWidth=Math.max(4,Number(o.thickness||2)*2.4);
 const bx=o.x2-ux*headLength,by=o.y2-uy*headLength,px=-uy,py=ux;
 return `${o.x2},${o.y2} ${bx+px*halfWidth},${by+py*halfWidth} ${bx-px*halfWidth},${by-py*halfWidth}`;
}
function objectNode(o){
 ...
 if(o.type==='rectangle') n=el('rect',Object.assign({x:o.x,y:o.y,width:o.w,height:o.h},common));
   ...
   const dx=o.x2-o.x1,dy=o.y2-o.y1,len=Math.max(0.001,Math.hypot(dx,dy)),headLength=Math.max(8,Number(o.thickness||2)*4.2);
   const ex=o.x2-(dx/len)*headLength*0.72,ey=o.y2-(dy/len)*headLength*0.72;
   g.appendChild(el('line',Object.assign({x1:o.x1,y1:o.y1,x2:ex,y2:ey,'stroke-linecap':'round'},common)));
   n=el('polygon',{points:arrowPolygon(o),fill:o.color||'#000',stroke:o.color||'#000','stroke-width':Math.max(1,Number(o.thickness||2)*0.35),'stroke-linejoin':'round'});
 }
...
function renderPalette(){
 palette.innerHTML='';
 DEFAULT_COLORS.forEach(c=>{ const b=document.createElement('button');b.className='swatch'+(c.toLowerCase()===activeColor.toLowerCase()?' active':'');b.style.background=c;b.title=c;b.addEventListener('click',()=>setColor(c));palette.appendChild(b); });
 const label=document.createElement('label');label.id='more-label';label.textContent='More colours';
 const picker=document.createElement('input');picker.type='color';picker.id='more-color';picker.value=activeColor;picker.addEventListener('input',()=>setColor(picker.value));label.appendChild(picker);palette.appendChild(label);
}
function setColor(c){ activeColor=c; const o=byId(selectedId); if(o){pushHistory();o.color=c;emit();} renderPalette();render(); }
function render(){
 svg.innerHTML='';
...
function beginInlineText(p){
 const input=document.createElement('input'); input.type='text'; input.className='inline-text';
 const fs=Number(args.font_size||18); input.style.left=p.x+'px';input.style.top=p.y+'px';input.style.fontSize=fs+'px';input.style.fontFamily=args.font==='Arial'?'Arial':'sans-serif';input.style.fontWeight=args.bold?'bold':'normal';input.style.fontStyle=args.italic?'italic':'normal';input.style.textDecoration=args.underline?'underline':'none';input.style.color=activeColor;input.placeholder='Type text';
 stage.appendChild(input); input.focus();
 let finished=false;
 function finish(save){ if(finished)return;finished=true;const text=input.value;input.remove();if(save&&text.trim()){pushHistory();const w=Math.max(40,text.length*fs*0.62),h=fs*1.35;const o={id:rid(),type:'text',x:p.x,y:p.y,w,h,rotation:0,text,font:args.font||'Arial',font_size:fs,bold:!!args.bold,italic:!!args.italic,underline:!!args.underline,color:activeColor,thickness:thickness()};objects.push(o);selectedId=o.id;render();emit();}}
...
svg.addEventListener('pointerdown',e=>{
 ...
 if(args.tool==='text'){ beginInlineText(p); return; }
...
"""
    ...
    return components.declare_component("pdf_annotation_component_v2", path=component_dir)
...
def show_annotation_editor(page_num, page_image, annotations, selected_id, undo_stack, controls):
    ...
    return component(
        ...
        **controls,
        key=f"annotation_editor_v2_page_{page_num}",
        default=None,
    )
