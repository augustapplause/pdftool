...
# Version : 0.3
# Date    : 2026-07-16
# Changes : Fixed annotation persistence by removing select-event reruns,
#           saving only create/modify/delete/text commits, and replaced
#           page dropdown with a slider.
# ============================================================
...
nav_prev_col, nav_slider_col, nav_next_col = st.columns([1, 4, 1])
...
with nav_slider_col:
    slider_page = st.slider(
        "Page",
        min_value=1,
        max_value=page_count,
        value=st.session_state.active_page_num + 1,
        step=1,
        key="page_slider",
    )
    slider_index = int(slider_page) - 1
    if slider_index != st.session_state.active_page_num:
        st.session_state.active_page_num = slider_index
...
st.caption(f"Showing page {page_num + 1} of {page_count}")
...
def get_annotation_component():
    ...
    component_dir = os.path.join(tempfile.gettempdir(), "pdf_fabric_annotation_component_v3")
...
function installCanvasEvents() {
  ...
  // Do not emit selection-only events to Streamlit. Selection changes should stay local;
  // otherwise Streamlit reruns during drawing/text editing and reloads stale state.
  ...
  canvas.on("mouse:up", function() {
    ...
    if (tempObject) {
      ...
      const ann = serializeObject(tempObject);
      const tooSmall = !ann || (Math.abs((ann.x1 || 0) - (ann.x0 || 0)) < 4 && Math.abs((ann.y1 || 0) - (ann.y0 || 0)) < 4);
      if (tooSmall) {
...
"""
    ...
    return components.declare_component("pdf_fabric_annotation_component_v3", path=component_dir)
    ...
    if task == "Convert PDF to XLSX":
        ...
        if event is not None:
            try:
                ...
                if (
                    clicked_page == page_num
                    and event_action in {"create", "modify", "delete", "text"}
