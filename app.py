# PDF Toolkit Version 2.0 - Refactor-safe task routing and redaction render fix
# Architecture: annotation editor owns browser edits; XLSX/redaction/text paths are isolated by task helpers.
# v2.1: added freehand pen to annotation editor.
# v2.1 Option 4 update: XLSX can use one shared column layout for all PDF pages.
# v2.0 fixes intermittent redaction preview rendering by replacing streamlit_image_coordinates with a local base64 HTML component.
...
with left_col:
    if task == "Convert PDF to XLSX":
        ...
        use_same_columns_all_pages = st.checkbox(
            "Use same column layout for all pages",
            value=bool(st.session_state.get("xlsx_use_same_columns_all_pages", False)),
            key="xlsx_use_same_columns_all_pages",
            help=(
                "When enabled, the column markers saved on Page 1 are used for every page. "
                "Turn this off if pages need different column layouts."
            ),
        )

        xlsx_rotation_degrees = st.selectbox(
            "Rotate PDF view for XLSX",
            options=[0, 90, 180, 270],
            index=[0, 90, 180, 270].index(int(st.session_state.get("xlsx_rotation_degrees", 0) or 0)),
            format_func=lambda value: "No rotation" if value == 0 else f"Rotate {value}° clockwise",
            key="xlsx_rotation_degrees",
            help=(
                "Use this when the PDF page is sideways. Column markers and extraction "
                "will use this rotated view."
            ),
        )

        layout_page_num = 0 if use_same_columns_all_pages else page_num
        current_columns = st.session_state.columns.get(layout_page_num, [])

        if use_same_columns_all_pages:
            st.caption(
                "Shared layout is enabled: the ruler edits the Page 1 column layout, "
        ...
        if xlsx_rotation_degrees:
            st.caption(
                f"XLSX rotation is enabled: pages are shown and extracted with a {xlsx_rotation_degrees}° clockwise rotation."
            )
            ...
            try:
                ...
                with pd.ExcelWriter(output_xlsx, engine="openpyxl") as writer:
                    ...
                    for pnum in range(total_pages):
                        ...
                        p = source_doc[pnum]
                        if use_same_columns_all_pages:
                            raw_columns = st.session_state.columns.get(0, [])
                        else:
                        ...
                        with status_area:
                            ...
                            if not raw_columns:
                                if use_same_columns_all_pages:
                                    st.write("Skipped: shared Page 1 column layout has no markers set.")
                                else:
                        ...
                        try:
                            ...
                            words = transform_words_for_xlsx_rotation(
                                words,
                                p,
                ...
                layout_mode_text = (
                    "Shared Page 1 column layout applied to all pages. "
                    if use_same_columns_all_pages
