...
def get_annotation_component():
    ...
    component_dir = os.path.join(tempfile.gettempdir(), "pdf_annotation_component_v3")
    os.makedirs(component_dir, exist_ok=True)
    index_path = os.path.join(component_dir, "index.html")
...
let args={}, objects=[], selectedId=null, action=null, draft=null, activeColor='#000000', undoStack=[], activeEditor=null;
...
function beginInlineText(p){
 ...
 function finish(save){
   ...
   if(save&&text.trim()){
     ...
     objects.push(o);selectedId=o.id;render();emit();
   } else { render(); }
 }
...
"""
    ...
    return components.declare_component("pdf_annotation_component_v3", path=component_dir)
...
def show_annotation_editor(page_num, page_image, annotations, selected_id, undo_stack, controls):
    ...
    return component(
        ...
        **controls,
        key=f"annotation_editor_v3_page_{page_num}",
        default=None,
    )
