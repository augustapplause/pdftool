...
# Version : 0.2
# Date    : 2026-07-16
# Changes : Rebuilt Fabric.js annotation component state handling,
#           fixed page navigation, in-canvas text entry, arrowheads,
#           independent shape resizing, and annotation persistence.
# ============================================================
...
nav_prev_col, nav_select_col, nav_next_col = st.columns([1, 3, 1])

with nav_prev_col:
    if st.button("Previous Page", disabled=st.session_state.active_page_num <= 0):
...
with nav_select_col:
    page_num = st.selectbox(
        ...
        index=st.session_state.active_page_num,
        format_func=lambda x: f"Page {x + 1}",
        key="page_selectbox",
    )
    if page_num != st.session_state.active_page_num:
        st.session_state.active_page_num = page_num
        st.rerun()

with nav_next_col:
    if st.button("Next Page", disabled=st.session_state.active_page_num >= page_count - 1):
        st.session_state.active_page_num += 1
        st.rerun()
...
def get_annotation_component():
    ...
    component_dir = os.path.join(tempfile.gettempdir(), "pdf_fabric_annotation_component_v2")
...
#hint { position: sticky; left: 0; top: 0; z-index: 10; min-height: 34px; box-sizing: border-box; background: rgba(255,255,255,0.96); padding: 8px 10px; font-size: 13px; color: #333333; border-bottom: 1px solid #dddddd; }
...
function fontSize() { return Math.max(8, Number(args.font_size || 18)); }

function updateHint() {
  const el = document.getElementById("hint");
  const t = tool();
  if (t === "select") el.textContent = "Select / Move: drag annotations, resize with handles, double-click text to edit, press Delete or Backspace to remove selected objects.";
  else if (t === "textbox") el.textContent = "Text: click directly on the PDF and type. Click outside the text when done. Text boxes have no fill and no border.";
...
function setCommon(obj, kind, id) {
  obj.set({ id: id || makeId(), kind: kind, transparentCorners: false, cornerSize: 9, borderColor: "#0066ff", cornerColor: "#ffffff", cornerStrokeColor: "#0066ff", objectCaching: false });
  return obj;
}
function createFabricObject(a) {
  const kind = a.kind || "textbox";
  const id = a.id || makeId();
  const stroke = a.custom_color || a.color_hex || "#ff0000";
  const width = Math.max(1, Number(a.line_width || 3));
  const x0 = Number(a.x0 || 0), y0 = Number(a.y0 || 0), x1 = Number(a.x1 || x0), y1 = Number(a.y1 || y0);
  if (kind === "textbox") {
    const txt = new fabric.IText(a.text || "", { left: x0, top: y0, fill: stroke, fontFamily: a.font_family === "Arial" ? "Arial" : "sans-serif", fontSize: Math.max(8, Number(a.font_size || 18)), fontWeight: a.bold ? "bold" : "normal", fontStyle: a.italic ? "italic" : "normal", underline: !!a.underline, backgroundColor: "", stroke: null, strokeWidth: 0, padding: 0, editable: true });
    txt.set({ custom_color: stroke, line_width: 1, font_family: a.font_family || "Sans Serif", bold_value: !!a.bold, italic_value: !!a.italic, underline_value: !!a.underline });
    return setCommon(txt, "textbox", id);
  }
  if (kind === "line" || kind === "arrow") {
    const line = new fabric.Line([x0, y0, x1, y1], { stroke: stroke, strokeWidth: width, fill: stroke, objectCaching: false, strokeUniform: true, originX: "left", originY: "top" });
    line.set({ custom_color: stroke, line_width: width });
    return setCommon(line, kind, id);
  }
  if (kind === "rectangle") {
    const rect = new fabric.Rect({ left: Math.min(x0, x1), top: Math.min(y0, y1), width: Math.max(1, Math.abs(x1 - x0)), height: Math.max(1, Math.abs(y1 - y0)), fill: "rgba(0,0,0,0)", stroke: stroke, strokeWidth: width, strokeUniform: true, objectCaching: false });
    rect.set({ custom_color: stroke, line_width: width });
    return setCommon(rect, kind, id);
  }
  if (kind === "circle") {
    const ell = new fabric.Ellipse({ left: Math.min(x0, x1), top: Math.min(y0, y1), rx: Math.max(1, Math.abs(x1 - x0) / 2), ry: Math.max(1, Math.abs(y1 - y0) / 2), fill: "rgba(0,0,0,0)", stroke: stroke, strokeWidth: width, strokeUniform: true, objectCaching: false });
    ell.set({ custom_color: stroke, line_width: width });
    return setCommon(ell, kind, id);
  }
  return null;
}
function normalizeModifiedObject(obj) {
  if (!obj || !obj.kind) return;
  if (obj.kind === "textbox") {
    const scaleY = Number(obj.scaleY || 1);
    if (Math.abs(scaleY - 1) > 0.001) obj.set({ fontSize: Math.max(8, Math.round(Number(obj.fontSize || 18) * scaleY)), scaleX: 1, scaleY: 1 });
    else if (Math.abs(Number(obj.scaleX || 1) - 1) > 0.001) obj.set({ scaleX: 1 });
    obj.setCoords(); return;
  }
  if (obj.kind === "rectangle") {
    const sx = Number(obj.scaleX || 1), sy = Number(obj.scaleY || 1);
    if (Math.abs(sx - 1) > 0.001 || Math.abs(sy - 1) > 0.001) obj.set({ width: Math.max(1, Number(obj.width || 1) * sx), height: Math.max(1, Number(obj.height || 1) * sy), scaleX: 1, scaleY: 1 });
    obj.setCoords(); return;
  }
  if (obj.kind === "circle") {
    const sx = Number(obj.scaleX || 1), sy = Number(obj.scaleY || 1);
    if (Math.abs(sx - 1) > 0.001 || Math.abs(sy - 1) > 0.001) obj.set({ rx: Math.max(1, Number(obj.rx || 1) * sx), ry: Math.max(1, Number(obj.ry || 1) * sy), scaleX: 1, scaleY: 1 });
    obj.setCoords(); return;
  }
}
...
function serializeObject(obj) {
  ...
  if (kind === "textbox") return { id: obj.id || makeId(), kind: "textbox", x0: Math.round(obj.left || 0), y0: Math.round(obj.top || 0), x1: Math.round((obj.left || 0) + obj.getScaledWidth()), y1: Math.round((obj.top || 0) + obj.getScaledHeight()), text: obj.text || "", custom_color: obj.custom_color || obj.fill || "#ff0000", line_width: 1, font_family: obj.font_family || (obj.fontFamily === "Arial" ? "Arial" : "Sans Serif"), font_size: Math.max(8, Math.round(Number(obj.fontSize || 18) * Number(obj.scaleY || 1))), bold: obj.fontWeight === "bold" || obj.bold_value === true, italic: obj.fontStyle === "italic" || obj.italic_value === true, underline: obj.underline === true || obj.underline_value === true };
  if (kind === "line" || kind === "arrow") { const p = lineEndpoints(obj); return { id: obj.id || makeId(), kind: kind, x0: Math.round(p.x0), y0: Math.round(p.y0), x1: Math.round(p.x1), y1: Math.round(p.y1), custom_color: obj.custom_color || obj.stroke || "#ff0000", line_width: Number(obj.line_width || obj.strokeWidth || 3) }; }
  if (kind === "rectangle") { const w = Number(obj.width || 1) * Number(obj.scaleX || 1), h = Number(obj.height || 1) * Number(obj.scaleY || 1); return { id: obj.id || makeId(), kind: "rectangle", x0: Math.round(obj.left || 0), y0: Math.round(obj.top || 0), x1: Math.round((obj.left || 0) + w), y1: Math.round((obj.top || 0) + h), custom_color: obj.custom_color || obj.stroke || "#ff0000", line_width: Number(obj.line_width || obj.strokeWidth || 3) }; }
  if (kind === "circle") { const w = Number(obj.rx || 1) * 2 * Number(obj.scaleX || 1), h = Number(obj.ry || 1) * 2 * Number(obj.scaleY || 1); return { id: obj.id || makeId(), kind: "circle", x0: Math.round(obj.left || 0), y0: Math.round(obj.top || 0), x1: Math.round((obj.left || 0) + w), y1: Math.round((obj.top || 0) + h), custom_color: obj.custom_color || obj.stroke || "#ff0000", line_width: Number(obj.line_width || obj.strokeWidth || 3) }; }
  return null;
}
function serializeAnnotations() { return canvas.getObjects().filter(o => o.kind).map(serializeObject).filter(Boolean); }
function emit(action) { if (isLoading || !canvas) return; const active = canvas.getActiveObject(); const payload = { page_num: args.page_num, page_key: args.page_key, action: action, annotations: serializeAnnotations(), selected_id: active && active.id ? active.id : null, nonce: nonce() }; lastServerAnnotationsJson = JSON.stringify(payload.annotations || []); setComponentValue(payload); }
function drawArrowHeadForObject(ctx, obj) {
  if (!obj || obj.kind !== "arrow") return;
  const p = lineEndpoints(obj), dx = p.x1 - p.x0, dy = p.y1 - p.y0;
  if (Math.sqrt(dx * dx + dy * dy) < 6) return;
  const angle = Math.atan2(dy, dx), head = Math.max(12, Number(obj.line_width || obj.strokeWidth || 3) * 5);
  ctx.save(); ctx.fillStyle = obj.stroke || "#ff0000"; ctx.beginPath(); ctx.moveTo(p.x1, p.y1); ctx.lineTo(p.x1 - head * Math.cos(angle - Math.PI / 7), p.y1 - head * Math.sin(angle - Math.PI / 7)); ctx.lineTo(p.x1 - head * Math.cos(angle + Math.PI / 7), p.y1 - head * Math.sin(angle + Math.PI / 7)); ctx.closePath(); ctx.fill(); ctx.restore();
}
function drawArrowHeads() { if (!canvas) return; const ctx = canvas.getContext(); canvas.getObjects().forEach(o => drawArrowHeadForObject(ctx, o)); }
function installCanvasEvents() {
  canvas.on("after:render", drawArrowHeads);
  canvas.on("selection:created", () => emit("select")); canvas.on("selection:updated", () => emit("select")); canvas.on("selection:cleared", () => emit("select"));
  canvas.on("object:modified", function(opt) { normalizeModifiedObject(opt.target); canvas.requestRenderAll(); emit("modify"); });
  canvas.on("text:editing:exited", function(opt) { normalizeModifiedObject(opt.target); emit("text"); });
  canvas.on("mouse:down", function(opt) {
    if (tool() === "select") return;
    const p = fabricPoint(opt);
    if (tool() === "textbox") {
      const obj = createFabricObject({ kind: "textbox", x0: p.x, y0: p.y, x1: p.x + 10, y1: p.y + 10, text: "", custom_color: color(), line_width: 1, font_family: args.font_family || "Sans Serif", font_size: fontSize(), bold: !!args.bold, italic: !!args.italic, underline: !!args.underline });
      canvas.add(obj); canvas.setActiveObject(obj); canvas.requestRenderAll(); obj.enterEditing(); if (obj.hiddenTextarea) obj.hiddenTextarea.focus(); return;
    }
    isDrawing = true; startPoint = p; tempObject = createFabricObject({ kind: tool(), x0: p.x, y0: p.y, x1: p.x, y1: p.y, custom_color: color(), line_width: lineWidth() });
  ...
  canvas.on("mouse:up", function() {
    if (!isDrawing) return;
    isDrawing = false;
    if (tempObject) { tempObject.selectable = true; tempObject.evented = true; tempObject.setCoords(); const ann = serializeObject(tempObject); const tooSmall = !ann || (Math.abs((ann.x1 || 0) - (ann.x0 || 0)) < 4 && Math.abs((ann.y1 || 0) - (ann.y0 || 0)) < 4); if (tooSmall) canvas.remove(tempObject); else canvas.setActiveObject(tempObject); canvas.requestRenderAll(); emit("create"); }
...
function loadCanvasFromArgs() {
  const width = Number(args.width || 800), height = Number(args.preview_height || 1000);
  isLoading = true; canvas.setWidth(width); canvas.setHeight(height); canvas.clear();
  fabric.Image.fromURL("data:image/png;base64," + args.preview_b64, function(img) { img.set({ left: 0, top: 0, selectable: false, evented: false }); img.scaleToWidth(width); img.scaleToHeight(height); canvas.setBackgroundImage(img, function() { (args.annotations || []).forEach(a => { const obj = createFabricObject(a); if (obj) canvas.add(obj); }); const selectedId = args.selected_id; if (selectedId) { const match = canvas.getObjects().find(o => o.id === selectedId); if (match) canvas.setActiveObject(match); } lastServerAnnotationsJson = JSON.stringify(args.annotations || []); loadedPageKey = args.page_key; isLoading = false; applyMode(); canvas.requestRenderAll(); setFrameHeight(800); }, { originX: "left", originY: "top" }); }, { crossOrigin: "anonymous" });
}
function render(argsIn) {
  args = argsIn || {}; updateHint();
  const width = Number(args.width || 800), height = Number(args.preview_height || 1000);
  document.getElementById("content").style.width = width + "px"; document.getElementById("content").style.minHeight = (height + 34) + "px";
  ...
  const incomingJson = JSON.stringify(args.annotations || []);
  const mustReload = loadedPageKey !== args.page_key || lastServerAnnotationsJson !== incomingJson;
  if (mustReload) loadCanvasFromArgs(); else { applyMode(); setFrameHeight(800); }
}
...
"""
    ...
    return components.declare_component("pdf_fabric_annotation_component_v2", path=component_dir)
...
def show_draggable_annotation_page(page_num, base_image, annotations, settings, selected_id=None, caption=None):
    ...
    result = component(
        page_num=page_num,
        page_key=f"page_{page_num}_{base_image.width}_{base_image.height}",
...
with left_col:
    if task == "Convert PDF to XLSX":
        ...
        if annotation_tool == "textbox":
            ...
            st.caption("Click directly on the PDF where the text should go, then type in place.")
