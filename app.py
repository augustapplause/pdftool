...
def readtext_with_quality(reader, pil_image, quality_mode="High Accuracy"):
    """Run EasyOCR with settings that keep more low-confidence text candidates."""
...
def ocr_page_words_for_xlsx_rotation(page, preview_zoom, rotation_degrees, ocr_zoom=None, quality_mode="High Accuracy"):
    ...
    quality_mode = str(quality_mode or "High Accuracy")
    if ocr_zoom is None:
        ocr_zoom = 1.75 if quality_mode == "Normal" else 3.0 if quality_mode == "High Accuracy" else 3.5
    ...
    try:
        ...
        processed_image = prepare_image_for_ocr(rotated_image, quality_mode)
        ocr_results = readtext_with_quality(reader, processed_image, quality_mode)

        if quality_mode == "High Accuracy / Slower":
            # Second pass on the unprocessed rotated image can recover text that
            # thresholding/sharpening occasionally damages. Merge by text + location.
            secondary_results = readtext_with_quality(reader, rotated_image, "High Accuracy")
            ocr_results = merge_ocr_results(ocr_results, secondary_results, ocr_zoom)
    finally:
        image.close()
    ...
    return easyocr_results_to_words(ocr_results, ocr_zoom)
...
def get_words_for_xlsx_extraction(page, pnum, zoom, rotation_degrees, status_area=None, ocr_quality_mode="High Accuracy", ocr_zoom=None):
    ...
    if ocr_zoom is None:
        ocr_zoom = 1.75 if ocr_quality_mode == "Normal" else 3.0 if ocr_quality_mode == "High Accuracy" else 3.5
...
with left_col:
    if task == "Convert PDF to XLSX":
        ...
        if xlsx_ocr_quality_mode == "Normal":
            ...
            xlsx_ocr_zoom = 3.5
        st.caption(f"OCR render zoom: {xlsx_ocr_zoom:.2f}x. Preview zoom remains {zoom:.2f}x, so ruler positions are unchanged.")
