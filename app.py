# PDF Toolkit - v0.4
# Annotation fixes: stable Fabric emits, in-place text, persistent arrows/lines, page slider under PDF.
...
def get_annotation_component():
    ...
    component_dir = os.path.join(tempfile.gettempdir(), "pdf_fabric_annotation_component_v4")
...
function createFabricObject(a) {
  ...
  if (kind === \"rectangle\") { const rect = new fabric.Rect({ left: Math.min(x0, x1), top: Math.min(y0, y1), width: Math.max(1, Math.abs(x1 - x0)), height: Math.max(1, Math.abs(y1 - y0)), fill: \"rgba(0,0,0,0)\", stroke: stroke, strokeWidth: width, strokeUniform: true, objectCaching: false }); rect.set({ custom_color: stroke, line_width: width }); return setCommon(rect, kind, id); }
  if (kind === \"circle\") { const ell = new fabric.Ellipse({ left: Math.min(x0, x1), top: Math.min(y0, y1), rx: Math.max(1, Math.abs(x1 - x0) / 2), ry: Math.max(1, Math.abs(y1 - y0) / 2), fill: \"rgba(0,0,0,0)\", stroke: stroke, strokeWidth: width, strokeUniform: true, objectCaching: false }); ell.set({ custom_color: stroke, line_width: width }); return setCommon(ell, kind, id); }
  return null;
}
...
function serializeObject(obj) {
  normalizeScaledObject(obj); const kind = obj.kind || \"textbox\";
  ...
  if (kind === \"rectangle\") return { id: obj.id || makeId(), kind: kind, x0: Math.round(obj.left || 0), y0: Math.round(obj.top || 0), x1: Math.round((obj.left || 0) + obj.width), y1: Math.round((obj.top || 0) + obj.height), custom_color: obj.custom_color || obj.stroke || \"#ff0000\", line_width: Number(obj.line_width || obj.strokeWidth || 3) };
  if (kind === \"circle\") return { id: obj.id || makeId(), kind: kind, x0: Math.round(obj.left || 0), y0: Math.round(obj.top || 0), x1: Math.round((obj.left || 0) + 2 * obj.rx), y1: Math.round((obj.top || 0) + 2 * obj.ry), custom_color: obj.custom_color || obj.stroke || \"#ff0000\", line_width: Number(obj.line_width || obj.strokeWidth || 3) };
  return null;
}
...
function installCanvasEvents() {
  ...
  canvas.on(\"mouse:move\", function(opt) { if (!isDrawing || !tempObject || !startPoint) return; const p = fabricPoint(opt); const kind = tempObject.kind; if (kind === \"line\" || kind === \"arrow\") tempObject.set({ x2: p.x, y2: p.y }); else if (kind === \"rectangle\") tempObject.set({ left: Math.min(startPoint.x, p.x), top: Math.min(startPoint.y, p.y), width: Math.max(1, Math.abs(p.x - startPoint.x)), height: Math.max(1, Math.abs(p.y - startPoint.y)) }); else if (kind === \"circle\") tempObject.set({ left: Math.min(startPoint.x, p.x), top: Math.min(startPoint.y, p.y), rx: Math.max(1, Math.abs(p.x - startPoint.x) / 2), ry: Math.max(1, Math.abs(p.y - startPoint.y) / 2) }); tempObject.setCoords(); canvas.requestRenderAll(); });
...
"""
    ...
    return components.declare_component("pdf_fabric_annotation_component_v4", path=component_dir)
