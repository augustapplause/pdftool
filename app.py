...
def get_annotation_component():
    ...
    component_dir = os.path.join(tempfile.gettempdir(), "pdf_fabric_annotation_component_v1")
...
function updateHint() { const el = document.getElementById("hint"); const t = tool(); if (t === "select") el.textContent = "Select / Move: drag annotations, resize with handles, press Delete or Backspace to remove selected objects."; else if (t === "textbox") el.textContent = "Text: click the PDF to place text. Text boxes have no fill and no border."; else el.textContent = "Draw: drag on the PDF to create the selected shape. Switch to Select / Move to reposition or resize it."; }
function fabricPoint(opt) { const pointer = canvas.getPointer(opt.e); return { x: pointer.x, y: pointer.y }; }
function setCommon(obj, kind, id) { obj.set({ id: id || makeId(), kind: kind, transparentCorners: false, cornerSize: 9, borderColor: "#0066ff", cornerColor: "#ffffff", cornerStrokeColor: "#0066ff" }); return obj; }
function createFabricObject(a) {
  ...
  if (kind === "textbox") { const txt = new fabric.IText(a.text || "", { left: x0, top: y0, fill: stroke, fontFamily: a.font_family === "Arial" ? "Arial" : "sans-serif", fontSize: Number(a.font_size || 18), fontWeight: a.bold ? "bold" : "normal", fontStyle: a.italic ? "italic" : "normal", underline: !!a.underline, backgroundColor: "", stroke: null, strokeWidth: 0, padding: 0 }); txt.set({ font_family: a.font_family || "Sans Serif", font_size: Number(a.font_size || 18), bold: !!a.bold, italic: !!a.italic, underline_value: !!a.underline, custom_color: stroke, line_width: 1 }); return setCommon(txt, "textbox", id); }
  if (kind === "line" || kind === "arrow") { const line = new fabric.Line([x0, y0, x1, y1], { stroke: stroke, strokeWidth: width, fill: stroke, objectCaching: false, strokeUniform: true }); line.set({ custom_color: stroke, line_width: width }); return setCommon(line, kind, id); }
  if (kind === "rectangle") { const rect = new fabric.Rect({ left: Math.min(x0, x1), top: Math.min(y0, y1), width: Math.abs(x1 - x0), height: Math.abs(y1 - y0), fill: "rgba(0,0,0,0)", stroke: stroke, strokeWidth: width, strokeUniform: true, objectCaching: false }); rect.set({ custom_color: stroke, line_width: width }); return setCommon(rect, kind, id); }
  if (kind === "circle") { const ell = new fabric.Ellipse({ left: Math.min(x0, x1), top: Math.min(y0, y1), rx: Math.abs(x1 - x0) / 2, ry: Math.abs(y1 - y0) / 2, fill: "rgba(0,0,0,0)", stroke: stroke, strokeWidth: width, strokeUniform: true, objectCaching: false }); ell.set({ custom_color: stroke, line_width: width }); return setCommon(ell, kind, id); }
  return null;
}
function lineEndpoints(obj) { const m = obj.calcTransformMatrix(); const p1 = fabric.util.transformPoint(new fabric.Point(obj.x1, obj.y1), m); const p2 = fabric.util.transformPoint(new fabric.Point(obj.x2, obj.y2), m); return { x0: p1.x, y0: p1.y, x1: p2.x, y1: p2.y }; }
function serializeObject(obj) {
  const kind = obj.kind || "textbox";
  if (kind === "textbox") return { id: obj.id || makeId(), kind: "textbox", x0: Math.round(obj.left || 0), y0: Math.round(obj.top || 0), x1: Math.round((obj.left || 0) + obj.getScaledWidth()), y1: Math.round((obj.top || 0) + obj.getScaledHeight()), text: obj.text || "", custom_color: obj.custom_color || obj.fill || "#ff0000", line_width: 1, font_family: obj.font_family || (obj.fontFamily === "Arial" ? "Arial" : "Sans Serif"), font_size: Number(obj.font_size || obj.fontSize || 18), bold: obj.fontWeight === "bold" || obj.bold === true, italic: obj.fontStyle === "italic" || obj.italic === true, underline: obj.underline === true || obj.underline_value === true };
  ...
  const br = obj.getBoundingRect(true, true); return { id: obj.id || makeId(), kind: kind, x0: Math.round(br.left), y0: Math.round(br.top), x1: Math.round(br.left + br.width), y1: Math.round(br.top + br.height), custom_color: obj.custom_color || obj.stroke || "#ff0000", line_width: Number(obj.line_width || obj.strokeWidth || 3) };
}
function serializeAnnotations() { return canvas.getObjects().filter(o => o.kind).map(serializeObject); }
...
function drawArrowHead(ctx, obj) { const p = lineEndpoints(obj); const angle = Math.atan2(p.y1 - p.y0, p.x1 - p.x0); const head = Math.max(12, Number(obj.line_width || obj.strokeWidth || 3) * 5); ctx.save(); ctx.fillStyle = obj.stroke || "#ff0000"; ctx.beginPath(); ctx.moveTo(p.x1, p.y1); ctx.lineTo(p.x1 - head * Math.cos(angle - Math.PI / 7), p.y1 - head * Math.sin(angle - Math.PI / 7)); ctx.lineTo(p.x1 - head * Math.cos(angle + Math.PI / 7), p.y1 - head * Math.sin(angle + Math.PI / 7)); ctx.closePath(); ctx.fill(); ctx.restore(); }
function installCanvasEvents() {
  canvas.on("selection:created", () => emit("select")); canvas.on("selection:updated", () => emit("select")); canvas.on("selection:cleared", () => emit("select")); canvas.on("object:modified", () => emit("modify")); canvas.on("text:changed", () => emit("text"));
  canvas.on("mouse:down", function(opt) { if (tool() === "select") return; const p = fabricPoint(opt); if (tool() === "textbox") { const textValue = args.text || ""; if (!textValue.trim()) return; const obj = createFabricObject({ kind: "textbox", x0: p.x, y0: p.y, x1: p.x + 10, y1: p.y + 10, text: textValue, custom_color: color(), line_width: 1, font_family: args.font_family || "Sans Serif", font_size: args.font_size || 18, bold: !!args.bold, italic: !!args.italic, underline: !!args.underline }); canvas.add(obj); canvas.setActiveObject(obj); canvas.requestRenderAll(); emit("create"); return; } isDrawing = true; startPoint = p; tempObject = createFabricObject({ kind: tool(), x0: p.x, y0: p.y, x1: p.x, y1: p.y, custom_color: color(), line_width: lineWidth() }); if (tempObject) { tempObject.selectable = false; tempObject.evented = false; canvas.add(tempObject); } });
  canvas.on("mouse:move", function(opt) { if (!isDrawing || !tempObject || !startPoint) return; const p = fabricPoint(opt); const kind = tempObject.kind; if (kind === "line" || kind === "arrow") tempObject.set({ x2: p.x, y2: p.y }); else if (kind === "rectangle") tempObject.set({ left: Math.min(startPoint.x, p.x), top: Math.min(startPoint.y, p.y), width: Math.abs(p.x - startPoint.x), height: Math.abs(p.y - startPoint.y) }); else if (kind === "circle") tempObject.set({ left: Math.min(startPoint.x, p.x), top: Math.min(startPoint.y, p.y), rx: Math.abs(p.x - startPoint.x) / 2, ry: Math.abs(p.y - startPoint.y) / 2 }); tempObject.setCoords(); canvas.requestRenderAll(); });
  canvas.on("mouse:up", function() { if (!isDrawing) return; isDrawing = false; if (tempObject) { tempObject.selectable = true; tempObject.evented = true; canvas.setActiveObject(tempObject); tempObject.setCoords(); const br = tempObject.getBoundingRect(true, true); if (br.width < 4 && br.height < 4) canvas.remove(tempObject); canvas.requestRenderAll(); emit("create"); } tempObject = null; startPoint = null; });
}
...
function render(argsIn) { args = argsIn || {}; updateHint(); const width = Number(args.width || 800), height = Number(args.preview_height || 1000); document.getElementById("content").style.width = width + "px"; document.getElementById("content").style.minHeight = (height + 34) + "px"; if (!window.fabric) { document.getElementById("hint").textContent = "Fabric.js could not be loaded. Check internet/CDN access or bundle fabric.min.js locally."; setFrameHeight(800); return; } if (!canvas) { canvas = new fabric.Canvas("pdf-canvas", { preserveObjectStacking: true, selection: true }); installCanvasEvents(); const oldRenderAll = canvas.renderAll.bind(canvas); canvas.renderAll = function() { oldRenderAll(); const ctx = canvas.contextTop; canvas.clearContext(ctx); canvas.getObjects().forEach(o => { if (o.kind === "arrow") drawArrowHead(ctx, o); }); }; } isLoading = true; canvas.setWidth(width); canvas.setHeight(height); canvas.clear(); fabric.Image.fromURL("data:image/png;base64," + args.preview_b64, function(img) { img.set({ left: 0, top: 0, selectable: false, evented: false }); img.scaleToWidth(width); img.scaleToHeight(height); canvas.setBackgroundImage(img, function() { (args.annotations || []).forEach(a => { const obj = createFabricObject(a); if (obj) canvas.add(obj); }); const selectedId = args.selected_id; if (selectedId) { const match = canvas.getObjects().find(o => o.id === selectedId); if (match) canvas.setActiveObject(match); } isLoading = false; applyMode(); canvas.requestRenderAll(); setFrameHeight(800); }, { originX: "left", originY: "top" }); }, { crossOrigin: "anonymous" }); }
...
'''
    ...
    return components.declare_component("pdf_fabric_annotation_component", path=component_dir)
...
with left_col:
    if task == "Convert PDF to XLSX":
        ...
        if annotation_tool == "textbox":
            text_col1, text_col2, text_col3, text_col4, text_col5 = st.columns([2.4, 1, 1, 1, 1])
            with text_col1:
                annotation_text = st.text_input("Text to place", key="annotation_text")
            with text_col2:
