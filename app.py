# PDF Toolkit Version 0.9 - Fabric annotation commit ordering fix
# PDF Toolkit - v0.7
# Annotation fixes: apply Fabric commit snapshots before rendering to avoid off-by-one select edits.
...
def get_annotation_component():
    ...
    component_dir = os.path.join(tempfile.gettempdir(), "pdf_fabric_annotation_component_v9")
...
function emit(action) {
  ...
  setComponentValue({
    ...
    action: action,
    commit_id: commitCounter,
    annotations: serializeAnnotations(),
    selected_id: active && active.id ? active.id : null,
    nonce: nonce()
  });
}
...
"""
    ...
    return components.declare_component("pdf_fabric_annotation_component_v9", path=component_dir)
...
def show_draggable_annotation_page(page_num, base_image, annotations, settings, selected_id=None, caption=None):
    ...
    result = component(
        ...
        key=f"pdf_fabric_annotation_component_v9_page_{page_num}",
        default=None,
    )
...
with left_col:
    if task == "Convert PDF to XLSX":
        ...
        annotation_component_key = f"pdf_fabric_annotation_component_v9_page_{page_num}"
